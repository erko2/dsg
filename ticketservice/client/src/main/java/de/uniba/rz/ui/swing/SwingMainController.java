...
public class SwingMainController {
	...
	public void acceptTicket(int id) {
		try {
			...
			mainFrame.showTicketDetails(model.getTicket(id));
		} catch (TicketException | IOException exec) {
	...
	public void searchTicket(String name, Type type, int offset, int limit) {
		try {
			List<Ticket> tickets = model.searchTicket(name, type, offset, limit);
	...
	public void refreshTicketList() {
		...
		} catch (TicketException | NullPointerException e) {
